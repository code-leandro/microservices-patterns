...
public class PedidoController {
    ...
    public ResponseEntity<String> port(@Value("${local.server.port}") String port) {
        String body = String.format("Requisição respondida pela instância executando na porta: %s", port);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<PedidoDto> atualizaStatus(@PathVariable Long id, @RequestBody PedidoDto pedidoDto){
        Pedido pedido = mapper.map(pedidoDto, Pedido.class);
        PedidoDto dto = service.atualiza(pedido);
        return ResponseEntity.ok(dto);
    }
